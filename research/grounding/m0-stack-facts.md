...
Two invocations, one machine, one seed. Run A shares the machine with a concurrent pytest session; run B has the machine to itself, on a clean tree at `203e124`.

| run | bits | ops | elapsed s | ops/s | expected ops | 60-bit wall s | stop | tag |
|---|---|---|---|---|---|---|---|---|
| A | 40 | 474179 | 0.687 | 690511.8 | 1218031 | — | solved | STRONG-EMPIRICAL |
| A | 60 | 100000000 | 145.223 | 688598.1 | 1166326476 | 1693.8 | cap-ops | STRONG-EMPIRICAL on ops/s, CONJECTURE on the wall |
| B | 40 | 474179 | 1.025 | 462709.4 | 1218031 | — | solved | STRONG-EMPIRICAL |
| B | 60 | 100000000 | 85.394 | 1171037.6 | 1166326476 | 996.0 | cap-ops | STRONG-EMPIRICAL on ops/s, CONJECTURE on the wall |

Both 60-bit walks stopped on `--cap-ops` at 10^8 group ops, above the 10^7 the bead asks for; the 30-minute cap was reached by neither. `extrapolated_wall_s` = expected ops / measured ops/s, so 1166326476 / 688598.1 = 1693.8 s ≈ 28.2 minutes for run A and 1166326476 / 1171037.6 = 996.0 s ≈ 16.6 minutes for run B. Both carry the CONJECTURE tag: each assumes a rate measured over the first 10^8 ops of one walk on one seed holds for the remaining ~1.07×10^9, and that the walk's expected length equals its birthday bound. The 1.7× spread between A and B is the load on the machine, not the algorithm — the op counts are identical across runs, and the only declared difference in conditions is the concurrent pytest session during A. That attribution is an inference from the two conditions, not an isolated measurement of scheduler effects. A rate for an unloaded core sits at the B end of that spread. Nothing here is a claim about any method's asymptotics, about other hardware, or about a compiled implementation's rate.

The 40-bit walk solved at 474179 ops in both runs against an expected 1218031, well inside the geometric spread of a single rho, and the walk is reproducible: same instance, same op count, same `x`. Its `x = 788702851439` on instance `49f48a74e2a82f681caf65ff2b64c9c8b22284723a76f821cabf294fa6f5cd9d` was accepted by the Tier-0 verifier in 0.061 s (A) and 0.057 s (B); the planted off-by-one `(x + 1) mod n` on the same instance is refused with reason `xP-ne-Q`. Both are asserted in `tests/integration/test_measure_rho40.py`, which runs the walk and the real `gp` verifier with no fault injection other than that off-by-one. `tests/unit/test_measure_rho_rate.py` pins the pure `rate_report(ops, elapsed_s)`: it raises `RefusedTooFewOps` naming `--cap-ops` below 10^6 ops, and above it reports ops/s with the extrapolated 60-bit wall tagged CONJECTURE. `scripts/mutation-check.sh` kills a wrongly solved discrete log (3 tests), a removed 10^6-op floor (4 tests) and a planted `x + 0` negative (1 test).
