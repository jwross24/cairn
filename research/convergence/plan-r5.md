...
- **State layer:** branch tree (`{id, parent, hypothesis, status∈{active,parked,refuted,
  ...
  - *Terminal-status invariant:* every opened branch and every opened claim is a linear
    ...
    `budget_preempt` or withdraw. *Why:* a subprocess that dies with its harness is neither
...
- **Gate layer (mechanical, immutable):** submission verifier (`xP==Q` subprocess or no
  ...
  - *Gate discipline:* every gate ships a planted-failure self-test (a fixture that must
    ...
    per-cycle audit line (§7), the egress renderer and its refusal predicate (§10) and every
    worker tool allow-list form the **gate bundle**: a
...
skill's declared cost profile, the spawning budget (the meet above), the launch's
hypothesis key and method
identity (and its statement hash where a claim exists; §3, §4), the launch revision's
self-test standing — certified and not yanked (§2) — and the launch's *ticket* — and
...
launching party writes reaches the gate except those five. The ticket is
...
failing closed; otherwise INCONCLUSIVE or REJECT. The cap binds where the floor lives
...
- **Egress rule.** What leaves the harness — a negative-results entry (§11), a human-queue
  ...
  for any instance — is the verifier's passing node (§13) or nothing, which is what "no
  submit, ever" (§4) names. The renderer and its refusal predicate are gate-bundle fields
...
- **M1 — single track.** Prover + independent skeptic + ladder, with the ladder's baseline
  ...
  no Tier-2 node is refused, and a
  ...
  in-sample check, the memory cap at the floor rungs and the memory-model check, patience
  ceiling,
