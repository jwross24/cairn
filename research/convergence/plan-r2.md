...
- **Skill identity and self-test shape.** A skill's identity is a typed bundle
  ...
  *after* the implementation revision is content-addressed. Tier-0 arithmetic skills verify
  ...
  cached, never a ticket, never evidence), results already cached under it are disowned by
  `salt` (§3) when the fault is found to reach them, and only an attributed new revision with
  a fresh certificate clears the yank. *Why:* the known-answer corpus is the self-test
  ...
  where nearly every refutation lives; and a revision that keeps minting OK results after its
  own self-test fails is a gate that fails open.
...
- **Every recipe is hashed by `(skill@version, inputs-by-hash, seed, tool-versions,
  ...
  - *Key construction:* the key is BLAKE3-256 over a canonical, typed, domain-separated
    ...
    field), enforced at write time and covered by a known-answer test vector. Two logically equal
...
- **Retention:** keep lineage + certificates forever (cheap, load-bearing); GC the giant
  ...
  - *Mechanism:* recipes, ledger rows, certificates and recorded divergences are GC
    ...
    gate run, to a checkpoint outside the orchestrator's write path — at M0 an operator-owned
    ...
    cannot truncate — and a log that does not extend the last checkpoint fails verification
    at startup and before any promotion. *Why:* "the orchestrator may rewrite the tree but
...
- **State layer:** branch tree (`{id, parent, hypothesis, status∈{active,parked,refuted,
  ...
  - *Ledger preflight:* before any branch is funded the ledger answers `Allowed | Blocked
    ...
    nothing broader; a broader blacklist needs a formal entry. A
...
- **Workers (Research-Mode prompt = template):** Librarian (lit-grounding: "tried before /
  ...
  read-only, claim-agnostic role template in the gate bundle. *Why:* context isolation is a
  property of the dispatch, not of the model; a parent model
...
- **Gate layer (mechanical, immutable):** submission verifier (`xP==Q` subprocess or no
  ...
  - *Gate discipline:* every gate ships a planted-failure self-test (a fixture that must
    ...
    (§7) and every worker tool allow-list form the **gate bundle**: a content-addressed bundle
    held in a separate
...
- **Tier 2 — many cores, hours:** larger linear algebra, rho up to ≈ 90 bits for
  *validation* (the ceiling is arithmetic: at ≈ 0.11 µs per iteration a negation-map rho
  at 0.886√n costs ≈ 30 core-hours at 80 bits, ≈ 10³ at 90 and ≈ 3×10⁴ at 100), big families,
  substantial
...
and the tier gate enforces cheap-before-expensive automatically. A cost profile names two
...
verification the remaining budget cannot cover. *Why:* a gate whose compute is unbudgeted
is a gate skipped under load — silent fail-open by a slower route. A spawned worker's budget
...
the tier below. A launch is refused when the declared tier exceeds the ticket's tier by more
than one, when the declared cost exceeds the remaining budget, or when the required ticket is
absent: for Tier 2 on an algorithmic claim the ticket is a ladder result table with verdict
KEEP (§6), with one scoped exception — a table with verdict KEEP_IN_SAMPLE is a Tier-2 ticket
valid for the 60-bit rung of the same hypothesis key and for no other launch; the ticket
records that purpose and the gate refuses a launch whose declared purpose differs; for
Tier 2 on a conjecture it is a counterexample-hunt record (§7) with verdict SURVIVED (plus
the P1 floor, if adopted); Tier 3 adds the justification, certificate plan and sign-off
named above. A refusal is a `TierRefused` record the ledger keeps, not an exception the
orchestrator can catch and retry around; a branch flagged by §8 holds no ticket above
Tier 1. *Why:* "each tier's admission ticket is a result from the tier below" is a gate only
if the ticket is a named node the gate can read; and KEEP requires the 60-bit rung while
that rung costs Tier-2 budget, so an unscoped rule is satisfiable by no honest claim.
...
gate's entropy commitment of step (1); the ladder refuses to start otherwise. (1) The ladder
...
derives the seed of trial `i` at size `s` as `H(nonce ‖ hypothesis hash ‖ s ‖ i)`, keeps
...
trial count, same instance stream — whose CI radius sets the decision band. The band is
always a ratio against the **best generic baseline the harness ships, measured in the same
harness on the same instances** — plain rho without the negation map (≈ 1.25√n ops, O(1)
memory) at M1, never BSGS; a negation-map rho (≈ 0.886√n, the constant §5 uses to price
Tier 2), once shipped as a baseline skill, is the baseline from the day it lands — and a
...
BSGS table for that size (≈ 1 GB at 50 bits) and measured by the harness as peak
...
column. (2b) The 60-bit rung is the completion-and-extrapolation rung: the model fitted on
...
out of sample is REJECT. A 60-bit generic trial is ≈ 1.3×10⁹ group ops (minutes in
compiled code, a quarter hour interpreted), so a full 10²-trial distribution check at 60
bits is Tier-2 work by its own declared profile, not Tier-1, and runs only when the tier
gate grants that budget. (2c) Verdicts are typed and fixed in the ladder plan: `REJECT`
(refutation floor failed, `xP ≠ Q` on any trial, memory cap exceeded, or an in-sample or
out-of-sample miss of the pre-registered model), `INCONCLUSIVE` (inside the band),
...
the hold-out run. (3) It reports group operations against `√n`, never seconds. (4) Two
...
(5) The result table (size, trials, mean and sd of ops, memory, success rate, fit to the
pre-registered model, out-of-sample prediction and its band, the nonce and seeds) is the
ladder's artifact, the Tier-2 ticket (§5) under the verdict rule of (2c), and the template
for a negative-results entry (§11). *Why two thresholds:* BSGS is the right floor because
a method that cannot beat a table lookup at 50 bits is not an advance, and the wrong bar
because a method costing 4.9×10⁷ ops and 900 MB would "beat BSGS" while being worse than
plain rho on both axes.
...
- **Calibration taxonomy (mandatory per claim):** `PROVEN` (formalized) · `STRONG-EMPIRICAL`
  ...
  - **"Green Lean" is the challenge/solution protocol.** The gate *compiles* the Challenge
    ...
    not exist yet, so M1 builds and probes it before the gate is trusted). The gate declares
...
- **Independent skeptic:** sees only the *statement*, never the prover's reasoning; rewarded
  ...
  - *By construction, at both layers.* Context: the skeptic is dispatched by the harness
    ...
    transcript or proof text — so no prover output, panel feedback or detection logic of the
...
- **Strong law of small numbers (hard gate):** a small-case pattern is a conjecture until it
  ...
  *resource* gate before Tier-2 spend) is decided at M1 with planted-false conjectures
  ...
  verdict ∈ {SURVIVED, KILLED, BUDGET_EXHAUSTED}}`; SURVIVED under the declared budget is
  the Tier-2 ticket for a conjecture (§5), KILLED writes the ledger entry of §4,
  BUDGET_EXHAUSTED is no ticket, and without this node the conjecture's tag ceiling is
...
- **Standing foundations-auditor (above the per-claim skeptic):** the per-claim skeptic
  ...
  nodes with the largest `justified_by` fan-in are drawn first. At the M0 parameters the
  ...
  nodes, a full re-run for Replayable ones — from the verification component escrowed at
  each node's launch (§5); where the escrow cannot fund `l`, the auditor draws what it can
...
- **M0 — the real slice.** Substrate schema (§3) + **one exemplar skill built end-to-end**
  ...
  - *Exemplar skill:* `toy_curve(bits, seed) → (p, a, b, n, P)` — a random prime `p` of
    ...
    *implementation* needs a non-PARI backend, an M0 install decision and not an
    ...
    curve-and-order cases (one at 150 bits with composite order) and one negative control
    (`n·Q ≠ O`); no second complete `(P, Q, x)` triple exists upstream and a locally
    completed one is `author_supplied` — plus a `randomized_postcondition` arm on every
  ...
  - *M0 schema includes:* the §3 recipe-key construction, attempt records, cache bits and
    ...
    tag history, a GC roots table, the budget ceiling (§5), and the gate-bundle pin (§4).
...
- **M1 — single track.** Prover + independent skeptic + ladder, with the ladder's baseline
  ...
  KEPT (its √2 on group operations at equal memory is a known answer); (k) the A/A null arm
  itself submitted as a claim returns INCONCLUSIVE, never KEEP or REJECT; (l) a Lean
  Solution matching its Challenge exactly with axioms inside the allow-list reaches PROVEN;
  (m) a reproducible Tier-1 measurement with its repro node is admitted by `justify` to
  STRONG-EMPIRICAL; and the tier-gate fixture: a `KEEP_IN_SAMPLE` table admits the 60-bit
  rung of its own hypothesis key and is refused for every other Tier-2 launch. Adds the
  ladder protocol (§6) with its pre-registered model, commit-ordered instance entropy,
  typed verdicts, memory cap, patience ceiling, A/A null arm and out-of-sample rung, the
  budget ceiling (§5), the counterexample-hunt record (§7),
  the formalization gate's challenge/solution protocol (§7) with its gate-compiled
  Challenge and planted-`sorry` and wrong-statement fixtures — M1 builds and probes the
...
- **M2 — memory.** Dead-end ledger (refuted-by-hypothesis-key vs parked) + formalizer +
  ...
  refuted one is never re-walked. Adds the ledger preflight with its measured-point reach and
  `supersedes_refuted_review` park, the hash-chained log and its head checkpoint (§3, §4),
...
- **M4 — scale-out + taste.** Parallel tracks, Librarian gate, problem queue + measured
  ...
  *Done when:* multi-track portfolio runs with ledger + calibration coherent, the
  ...
  stress fixture. Decides the selector audit protocol and the
...
- **P4 · Operational alarms (M1–M2).** A split-conformal anomaly flag on ladder and
  ...
  `log(measured/declared cost)`. Strategy-layer alarms only; the per-run budget ceiling of
