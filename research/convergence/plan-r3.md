...
- **Skill identity and self-test shape.** A skill's identity is a typed bundle
  ...
  *after* the implementation revision is content-addressed. Tier-0 arithmetic skills verify
  ...
  predicate over recipe-key fields (the revision, plus a declared input range where the fault
  is input-dependent) and every attempt inside that reach is disowned (§3), and only an
  ...
  own self-test fails is a gate that fails open; and an uncertified revision is a yanked one
...
- **Every recipe is hashed by `(skill@version, inputs-by-hash, seed, tool-versions,
  ...
  - *Cache bits on every node:* `do_not_cache` (never memoize), `skip_cache_lookup` (run
    ...
    attempts outside the named reach stand. For a `Replayable` recipe, two successful attempts with
...
- **Retention:** keep lineage + certificates forever (cheap, load-bearing); GC the giant
  ...
  - *Mechanism:* recipes, ledger rows, certificates and recorded divergences are GC
    ...
    gate run, to a checkpoint outside the orchestrator's write authority — at M0 a file
...
- **State layer:** branch tree (`{id, parent, hypothesis, status∈{active,parked,refuted,
  ...
  - *Ledger preflight:* before any branch is funded the ledger answers `Allowed | Blocked
    ...
    `Blocked`. Every REFUTED/PARKED entry carries
    ...
    be the party that re-measures it.
...
- **Gate layer (mechanical, immutable):** submission verifier (`xP==Q` subprocess or no
  ...
  - *Gate discipline:* every gate ships a planted-failure self-test (a fixture that must
    ...
    class predicate (§7), the auditor's `f`, `bits`, critical-set cutoff and cadence
...
identity with the launch's and refuses a mismatch, so a method laddered as A cannot spend
Tier 2 as B under a repointed branch. A launch is refused when the declared tier exceeds the
ticket's tier by more than one, when the declared cost exceeds the remaining budget, or when
the required ticket is absent. The ticket lattice is total, so the gate has a branch for
...
around; a branch flagged by §8 holds no ticket above Tier 1. *Why:* "each tier's admission
...
model and never a curve fitted afterward. The hypothesis object's hash and its method
identity (skill@version and canonical parameters) are named in the ladder's dispatch record;
the ladder runs exactly that identity — every trial recipe key is built from it, a different
identity is refused before any trial, and a mismatched identity is a planted-failure class
...
comparison protocol as one versioned object: the estimator (a CI on the ratio of claim cost
to baseline cost — the source's median-CI form or a mean CI, named), the pairing (claimant
...
INCONCLUSIVE with margin — the 10² figure is a floor, and at rho's sd ≈ 0.5·mean the ratio CI
at 10² trials is about ±15 %, which puts the √2 control at the band's edge — and each rung's
...
a speedup is KEPT only if the whole claim CI clears `1 + 2·radius` (floor 1%, the guard for
a degenerate null arm) on group operations *and* peak memory both stays under the rung's
...
the ladder plan's tolerance is REJECT. The harness records CPU-seconds per trial beside the
count, and a trial whose CPU time exceeds what its counted operations would take at the
per-operation rate measured on the A/A arm of the same rung, by more than the ladder plan's
tolerance, marks the rung INCONCLUSIVE — never KEEP, never REJECT — and surfaces on the
human queue: the counter cannot see arithmetic done around it, but the clock can. A method
...
(5) The result table (size, trials, mean and sd of ops, CPU-seconds, memory, success rate,
fit to the pre-registered model, out-of-sample prediction and its band, the nonce and seeds,
the hypothesis hash and method identity it ran, the gate-bundle hash it ran under) is the
ladder's artifact, the Tier-2 ticket (§5) under the verdict rule of (2c), and the template
for a negative-results entry (§11); a verdict is never recomputed under a later bundle. *Why
...
- **Calibration taxonomy (mandatory per claim):** `PROVEN` (formalized) · `STRONG-EMPIRICAL`
  ...
  - **The tag is derived, never set.** Each evidence kind has a maximum class it may
    ...
    → STRONG-EMPIRICAL, where "repro node" means the reproducibility record the §3 policy
...
- **Independent skeptic:** sees only the *statement*, never the prover's reasoning; rewarded
  ...
  - *By construction, at both layers.* Context: the skeptic is dispatched by the harness
    ...
    the Challenge's statement hash and the gate-run verdict only, never the Solution blob or
...
- **M0 — the real slice.** Substrate schema (§3) + **one exemplar skill built end-to-end**
  ...
  `justify` returns STRONG-EMPIRICAL from a synthetic `ladder_table` evidence node whose
  attempt carries a passed reproducibility record, returns the CONJECTURE ceiling for the
  ...
  - *M0 schema includes:* the §3 recipe-key construction, attempt records, cache bits and
    ...
    table, the budget ceiling (§5), and the gate-bundle pin (§4).
...
- **M1 — single track.** Prover + independent skeptic + ladder, with the ladder's baseline
  ...
  `review_verdict` recorded against the Challenge's statement hash through the human path,
  ...
  hypothesis key or method identity differs from the table's, and a claim whose hypothesis
  ...
  A/A null arm and out-of-sample rung, the budget ceiling (§5), the counterexample-hunt
...
- **M2 — memory.** Dead-end ledger (refuted-by-hypothesis-key vs parked) + formalizer +
  ...
  refutation without a null arm parks the proposal with `null_control_pending` and the
  blocker clears when the re-measurement lands; a log truncated to an earlier valid prefix
...
- **M4 — scale-out + taste.** Parallel tracks, Librarian gate, problem queue + measured
  ...
  *Done when:* multi-track portfolio runs with ledger + calibration coherent, the
  ...
  stress fixture, and the foundations auditor's audit record names its critical set, its
