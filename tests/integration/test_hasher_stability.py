...
def test_fresh_builds_and_single_edit_pairs(pinned_bundle, tmp_path):
    ...
    for label, (source, equal) in pairs.items():
        digest = build_hash(tmp_path / label, gate, source)
        print(
            "".join(
                difflib.unified_diff(SOURCE.splitlines(True), source.splitlines(True), fromfile="base", tofile=label)
            )
        )
