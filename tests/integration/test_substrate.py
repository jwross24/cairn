...
from cairn.substrate import GradeError, HashCollision, HashMismatch, Substrate, UnknownNode, WriterAlreadyOpen

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from _substrate_helpers import ENV_MANIFEST_HASH, IDENTITY_A, IDENTITY_B, SELFTEST_SUMMARY, TRANSCRIPT_HASH, launch, open_writer, recipe  # noqa: E402
...
def test_second_writer_in_process_is_refused_until_close(writer, tmp_path, db_snapshot):
    with pytest.raises(WriterAlreadyOpen):
...
def test_running_attempt_is_never_served(writer, db_snapshot):
    key = writer.put_recipe(recipe(1))
    ...
    db_snapshot(writer.conn, "running")
    assert writer.get_attempt(attempt)["status"] == "RUNNING"
    assert writer.serve(key) is None
...
def populated(writer):
    ...
    return writer, {"key": key, "attempt": attempt, "manifest": manifest, "identity": identity, "cert": cert}
...
def test_receipts_are_append_only(writer, db_snapshot):
    receipt = {name: ("aa" * 32 if name.endswith("hash") or name.endswith("digest") else 1) for name in substrate.RECEIPT_FIELDS}
    digest = writer.put_receipt(receipt)
    with pytest.raises(sqlite3.IntegrityError, match="append-only"):
        writer.conn.execute("UPDATE receipts SET exit_status = 3 WHERE receipt_hash = ?", (digest,))
    with pytest.raises(sqlite3.IntegrityError, match="append-only"):
        writer.conn.execute("DELETE FROM receipts WHERE receipt_hash = ?", (digest,))
    db_snapshot(writer.conn, "receipts")
