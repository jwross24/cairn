...
def test_mr_w_moving_an_endpoint_inside_the_scope_is_a_coverage_violation(case, data):
    ...
    result = justify.justify({**evidence, "population": narrowed}, _statement(scope), ctx)
    assert isinstance(result, justify.CoverageViolation) and result.field == field


@given(covering_case(), st.data())
def test_mr_a_an_assumption_outside_the_scope_is_always_a_coverage_violation(case, data):
    scope, population, evidence, ctx = case
    outside = data.draw(
    ...
    extra = {
        **population,
        "assumption_set": frozenset(population["assumption_set"]) | {factories.assumption_id(outside)},
    }
...
def node_sequence(draw):
    scope = draw(factories.scopes())
    population = draw(factories.covering_populations(scope))
    kinds = draw(st.lists(st.sampled_from(factories.JUSTIFYING), min_size=1, max_size=5))
    ctx = justify.Context(repro_passed=draw(st.sampled_from((None, True))), approved=True)
    nodes = [
        _evidence(
            kind,
            verdict,
...
def test_mr_m_the_derived_class_never_falls_as_nodes_are_appended(case):
    ...
    classes = [justify.rank(_derived(scope, nodes[: n + 1], ctx)) for n in range(len(nodes))]
...
def test_mr_l_a_class_above_the_kind_ceiling_is_refused_and_one_below_is_not(case, offered):
    scope, _, evidence, ctx = case
...
def _expected_cap(summary, inputs):
    origins = [value for entry in summary["corpus_origins"].values() for value in entry.values()]
...
def test_mr_c_a_producer_with_no_covering_basis_caps_at_conjecture(case, summary):
    ...
    if not (inputs and _expected_cap(summary, inputs)):
        return
    with_producer = justify.Context(**{**ctx.__dict__, "producer_summary": summary, "attempt_inputs": inputs})
    result = justify.justify(evidence, _statement(scope), with_producer)
    if not isinstance(result, justify.Justification):
        return
...
def test_justify_never_raises_on_an_arbitrary_population(population, scope):
    ...
    result = justify.justify(evidence, _statement(scope), justify.Context(repro_passed=True))
