...
import _ci_lanes
import pytest
from _ci_lanes import LEAN_TEST_PATHS, validate_manifest
...
@pytest.mark.parametrize(("lane", "passed", "deselected"), [("all", 2, 0), ("python", 1, 1), ("lean", 1, 1)])
...
def test_default_runs_every_test_and_lane_collections_are_a_disjoint_union(pytester):
    ...
    default.assert_outcomes(passed=2)
    populations = {}
    for lane in ("all", "python", "lean"):
    ...
    assert populations["all"] == populations["lean"] | populations["python"]


@pytest.mark.parametrize("lane", ["python", "lean"])
def test_a_selected_failure_keeps_the_lane_red(pytester, lane):
    _suite(pytester)
    relative = "tests/integration/test_lean_toolchain.py" if lane == "lean" else "tests/unit/test_unlisted.py"
