...
| 4 | Gates (`bundle.py`, `gateplan.py`, `ladderplan.py`, `verifier.py`, `tiergate.py`, `runner.py`) | the gate bundle: objects `allow_lists`, `auditor`, `gate_plan`, `ladder_plan`, `lean`, `role_templates`, `tiers`, `verifier`, `waivable_checks`, `verifier_script` (`src/cairn/gp/verify.gp`), `lake_manifest` (`lean/lake-manifest.json`), `challenge_prelude` (`bundle/challenge_prelude.lean`), `challenge_renderer` (`src/cairn/challenge.py`), `container` (`bundle/container.json`), `container_file` (`bundle/Containerfile`); the verifier is `gp`, Python invokes | bundle hash under the operator pin; ticket hash | `gate_run` with gate names `canon_kat`, `verifier`, `tier_gate`, `self_test`, `gate_plan`, `bundle_open`, `ladder_plan`, `challenge_render` | `bundle`, `gate`, `ladder` | bundle hash ≠ pin, plan-step verdict ≠ expected, `xP ≠ Q`, tier two above, uncertified or yanked revision, budget ceiling |
