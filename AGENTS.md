...
- `.githooks/pre-commit` runs `scripts/check.sh --fast --paths <the commit's staged paths>`,
  ...
  `--ci-lane solution`, `--ci-lane solution-plan`, and `--ci-lane container`. An index that
...
`--ci-lane solution`, `--ci-lane solution-plan`, and `--ci-lane container`; their test sets are disjoint and exhaustive.
With no lane argument, the local check runs all five sets and requires Docker for the container tests.
