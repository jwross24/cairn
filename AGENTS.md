...
- `.githooks/pre-commit` runs `scripts/check.sh --fast --paths <the commit's staged paths>`,
  ...
  tree-wide run belongs to CI, which calls `scripts/check.sh --ci-lane python`, `--ci-lane lean`, and `--ci-lane solution`. An index that
...
`--ci-lane solution`; their test sets are disjoint and exhaustive. With no lane argument,
the local check runs all three sets.
